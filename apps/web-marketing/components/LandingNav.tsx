    ...
    >
      <div className="max-w-6xl mx-auto px-5 h-14 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          <a href={`${APP_URL}/login`} className="text-sm text-[#94A3B8] hover:text-white transition-colors hidden sm:block">Sign in</a>
          <a
      ...
      {open && (
        <div className="md:hidden border-t px-5 py-4 space-y-1" style={{ background: '#0D1421', borderColor: '#1E2D45' }}>
          ...
          <div className="pt-2 border-t" style={{ borderColor: '#1E2D45' }}>
            <a href={`${APP_URL}/login`} onClick={() => setOpen(false)} className="block py-2.5 text-sm text-[#CBD5E1] hover:text-white transition-colors">Sign in</a>
          </div>
        </div>
      )}
    </nav>
